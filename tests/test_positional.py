...
from numeral_system import positional, exceptions
...
class PositionalTestCase(TestCase):
    ...
    @parameterized.expand(
        [(randint(-9999, 9999), base) for _ in moves.range(5) for base in moves.range(2, positional.max_base())]
    )
    ...
    def test_check_digits_with_custom_alphabet_and_sign(self, number, base, expected):
        ...
        self.assertEqual(positional.decode(converted, base, alphabet=alphabet, sign_literal='@'), number)
